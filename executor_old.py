...
def check_completion_fast(artifacts: Dict[str, Any], page_text: str = "", transcription_text: str = "") -> Optional[Dict[str, Any]]:
    ...
    if extracted_artifacts:
        for key in extracted_artifacts:
            value = artifacts[key]
            if isinstance(value, (str, int, float)) and value not in ['', None]:
                logger.info(f"[FAST_CHECK] Simple extracted value found: {key} = {value}")
                return {"complete": True, "reason": "extracted_value_present"}
    ...
    if has_dataframe and needs_calculation and not any('statistics' in str(v) for v in artifacts.values()):
        logger.info(f"[FAST_CHECK] Have dataframe, need calculation, no statistics yet")
        return {"complete": False, "reason": "need_calculation", "needs_more": True}
    ...
    if (has_image or has_pdf) and needs_vision:
        ...
        if not has_vision_result:
            logger.info(f"[FAST_CHECK] Have image/PDF, need vision/OCR, no result yet")
            return {"complete": False, "reason": "need_vision_processing", "needs_more": True}
    ...
    if has_html and needs_extraction:
        has_extracted = any('extracted' in str(k).lower() for k in artifacts.keys())
        if not has_extracted:
            logger.info(f"[FAST_CHECK] Have HTML, need extraction, no result yet")
            return {"complete": False, "reason": "need_data_extraction", "needs_more": True}
...
async def check_plan_completion(plan_obj: Dict[str, Any], artifacts: Dict[str, Any], page_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in artifacts.items():
        if 'transcribe_audio_result' in key and isinstance(value, dict):
    ...
    if fast_result is not None:
        if fast_result.get("complete"):
            logger.info(f"[FAST_COMPLETE] {fast_result['reason']}")
            return {
                "answer_ready": True,
        ...
        elif fast_result.get("needs_more"):
            logger.info(f"[FAST_INCOMPLETE] {fast_result['reason']}")
            return {
                "answer_ready": False,
                "needs_more_tasks": True,
                "reason": fast_result['reason'],
                "recommended_next_action": "generate_tasks"
            }
...
async def format_artifact(artifact_key: str, artifact_value: Any, page_instructions: str) -> Any:
    ...
    if isinstance(artifact_value, dict):
        # Dict artifacts with HTML, text, or other nested data
        if 'html' in artifact_value or 'text' in artifact_value or 'rendered_divs' in artifact_value:
            should_process = True
    elif isinstance(artifact_value, str) and len(artifact_value) > 50:
        # Long strings that might contain extractable information
        if '<' in artifact_value or 'html' in artifact_value.lower():
            should_process = True
...
Respond with ONLY the value, no explanation."""
    ...
    try:
        response = await call_llm(prompt, system_prompt, 500, 0)
        ...
        if extracted and extracted != artifact_str[:100]:
            logger.info(f"[ARTIFACT_FORMAT] Extracted '{extracted}' from {artifact_key}")
            return extracted
        else:
            logger.info(f"[ARTIFACT_FORMAT] No extraction performed for {artifact_key}")
            return artifact_value
            
    except Exception as e:
        logger.warning(f"[ARTIFACT_FORMAT] Failed to format {artifact_key}: {e}")
        return artifact_value
